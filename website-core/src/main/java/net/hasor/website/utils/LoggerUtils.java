...
public class LoggerUtils {
    ...
    static {
        InputStream in = null;
        try {
            in = LoggerUtils.class.getResourceAsStream("/logger_messages.properties");
            errorProp.load(in);
        } catch (Exception e) {
    ...
    private LoggerUtils(String errorCode) {
        if (errorCode != null) {
    ...
    public static LoggerUtils create(String errorCode) {
        if (errorCode != null && !errorProp.containsKey(errorCode)) {
            logger.error("not found errorCode = " + errorCode);
        }
        return new LoggerUtils(errorCode);
    }
    public static LoggerUtils createSMS() {
        return createSMS(null);
    }
    public static LoggerUtils createSMS(String errorCode) {
    ...
    public LoggerUtils addMessage(Message errorMsg) {
        if (errorMsg == null) {
            put("messageCode", 0);
